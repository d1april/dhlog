...
LogFileOutput.prototype.addEntry = function (entry) {
	...
	var bytes = new Buffer(line.join(' ') + '\n');
	ws.write(bytes);
...
LogFileOutput.prototype.cycleFiles = function () {
	...
	try {
		...
		console.error('Failed to cycle log files ' + this.filePath, e);
	}


};

LogFileOutput.prototype.close = function (callb) {
	if(this.ws) {
		this.ws.end(callb);
		this.ws = null;
	} else {
